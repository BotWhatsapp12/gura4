...
break
  case 'ttp':  
if (args.length < 1) return reply('Teksnya mana? Contoh .ttp GuraCans')
ttp = args.join(' ')
anu1 = await getBuffer(`https://api.xteam.xyz/ttp?file&text=${ttp}`)
                    gura.sendMessage(from, anu1, image, {quoted: dep, caption : '.sticker'})
					reply(mess.wait)
break   
case 'add':
		if (!isGroup) return reply(mess.only.group)
...
case 'yts': case 'youtubesearch': case 'ytsearch':{
									...
									break
case 'attp':
				if (args.length < 1) return reply(`*Teksnya Mana ?*\n*Contoh ${prefix}attp ArulGanz*`)
				attp2 = await getBuffer(`https://api.xteam.xyz/attp?file&text=${body.slice(6)}`)
				gura.sendMessage(from, attp2, sticker, {quoted: ftrol})
				break
case 'bc':
            case 'broadcast':
